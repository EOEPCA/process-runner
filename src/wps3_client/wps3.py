...
@click.pass_context
def entry(ctx, application_package_url, wps_endpoint, result_method, conf):

    wps_params = {ctx.args[i][2:]: ctx.args[i+1] for i in range(0, len(ctx.args), 2)}
    
    main(wps_params, application_package_url, wps_endpoint, result_method, conf)

def main(wps_params, application_package_url, wps_endpoint, result_method, conf):
